...
export default function IncomeTimelinePage() {
  ...
  const { categoryData, categories, maxValue, availablePeriods } = useMemo(() => {
    const yearTransactions = transactions.filter(t => {
      ...
      return selectedYears.includes(year) && t.type === 'income'
    })
  ...
  const { selectedCategories, toggleCategory } = useSelectedCategories(allCategories, 'selected-income-timeline-sources')
  ...
  return (
    <div className="relative">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Income Timeline</h1>
          <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">
            Track your income evolution by source over time
          </p>
        </div>
        {years.length > 0 && (
      ...
      {transactions.length === 0 ? (
        <div className="bg-white dark:bg-gray-800 rounded-xl p-8 text-center border border-gray-200 dark:border-gray-700 shadow-theme-sm">
          <p className="text-gray-500 dark:text-gray-400">No transactions yet. Upload a statement to see your income timeline.</p>
        </div>
      ) : (
        <>
          {/* Category Filter */}
          <div className="bg-white dark:bg-gray-800 rounded-xl p-4 mb-6 border border-gray-200 dark:border-gray-700 shadow-theme-sm">
            <div className="flex items-center justify-between mb-3">
              <p className="text-sm text-gray-500 dark:text-gray-400">Select sources to display:</p>
          ...
          {selectedCategories.length > 0 && (
            <div className="bg-white dark:bg-gray-800 rounded-xl p-6 mb-6 border border-gray-200 dark:border-gray-700 shadow-theme-sm">
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-semibold text-gray-900 dark:text-white">Combined Income Overview</h3>
                <div className="flex flex-wrap gap-2">
                  ...
                  <div className="flex items-center justify-between mb-4">
                    ...
                    {trend && (
                      <div className={`flex items-center gap-1 text-sm ${
                        trend.increasing ? 'text-success-500' : 'text-error-500'
                      }`}>
                        {trend.increasing ? (
        ...
        {selectedCategories.length === 0 && (
          <div className="bg-white dark:bg-gray-800 rounded-xl p-8 text-center border border-gray-200 dark:border-gray-700 shadow-theme-sm">
            <p className="text-gray-500 dark:text-gray-400">Select at least one source to view the timeline.</p>
          </div>
        )}
      </>
    )}
